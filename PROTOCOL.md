...
Wire protocol for official SDKey clients (TypeScript, Python, C++, C#, …). All languages must use the same byte layouts and verification order.

**Protocol version:** `1`  
**Package:** [`sdkey`](https://pypi.org/project/sdkey/) (Python) and [`@sdkey/sdk`](https://www.npmjs.com/package/@sdkey/sdk) (TypeScript) implement this document.
...
Inner plaintext (before AES-GCM seal) — omit `hwid` when unused:
...
**No password.** Username + new license key only. New key’s `subscriptionTier` must be **greater than** the user’s current tier.
...
### Auth failure shape — text in `error`
...
Also: `LICENSE_REQUIRED`, `INVALID_CREDENTIALS`, `USERNAME_TAKEN`, `USER_NOT_FOUND`, `TIER_NOT_HIGHER`, plus shared codes such as `APP_OUTDATED`, `APP_DISABLED`, `IP_BANNED`, `HWID_BANNED`.
...
## Developer tooling (not part of this client)

Cookie or `Authorization: Bearer sdk_live_…` management APIs are out of scope for this package.
